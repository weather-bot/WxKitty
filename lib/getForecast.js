...
const parseForeignForecastMsg = require('../message/parseForeignForecastMsg');

const {
    isTaiwanArea,
    isTime,
    isForeignArea_Forecast
} = require('./keywords');
...
async function getForecast(msg) {
    ...
    const foreignArea = isForeignArea_Forecast(msg);
    //console.log(foreignArea);
    ...
    if (taiwanArea != null) {
        ...
        return "查不到此時間，目前臺灣縣市只支援 36小時內預報。請試著縮短時間，例如明天14:00、後天早上等，或是改變時間表示法，5月23號可以寫5-23，因為套件支援英文輸入，23/5才是可辨識的英文輸入。";
    } else if (foreignArea != null) {
        // If msg includes foreign city name and time is in 24 hours
        if (new Date(msgTime.epoch).getDate() == (new Date(currentEpoch).getDate() + 1)) {
            const obj = await getForeignForcast(foreignArea, msgTime);
            if (obj != null) {
                return parseForeignForecastMsg(obj);
            }
        }
        return "查不到此時間，目前只支援外國城市24小時內預報。請試著縮短時間，例如明天14:00，或是改變時間表示法，5月23號可以寫5-23，因為套件支援英文輸入，23/5才是可辨識的英文輸入。";
    } else {
        return "查不到此地區，請輸入台灣行政區名稱或外國城市名。目前只支援外國城市24小時內和臺灣縣市的36小時內預報";
    }
}
...
async function get7dayForecast(loc, msgTime) {
    try {
        ...
        const id = allTimeData.findIndex(e => {
...
async function getForeignForcast(city, msgTime) {
    try {
        const url = `http://opendata.cwb.gov.tw/opendataapi?` +
            `dataid=${list["worldCityForecast"]}&authorizationkey=${config.cwbKey}`;
        const res = await axios.get(url);
        const dataXml = res.data;
        const dataJson = JSON.parse(parser.toJson(dataXml));
        try {
        ...
        let forecast = {};
        location.forEach(e => {
            if (e.locationName == city) {
                forecast = {
                    "area": city,
                    "Stime": new Date(e.weatherElement[0].time.startTime),
                    "Etime": new Date(e.weatherElement[0].time.endTime),
