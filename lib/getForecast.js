...
async function getForecast(msg) {
    ...
    if (taiwanArea != null) {
        ...
        // If msg includes foreign city name and time is in 24 hours   //24+19??
        if (deltaTime > 0 && deltaTime < 60 * 60 * 43 * 1000) {
            const obj = await getForeignForcast(foreignArea, msgTime);
            if (obj != null) {
                return parseForeignForecastMsg(obj) + (new Date(currentEpoch)) + msgTime.time;
            }
        }
    ...
    try {
        ...
        location.forEach(e => {
            if (e.locationName == city) {
                forecast = {
                    "area": city,
                    "Stime": e.weatherElement[0].time.startTime,
                    "Etime": e.weatherElement[0].time.endTime,
