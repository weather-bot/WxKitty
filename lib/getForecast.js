...
async function getForecast(msg) {
    ...
    if (taiwanArea != null) {
        ...
        return "查不到此時間，目前只支援臺灣縣市的36小時內預報";
    } else {
        return "查不到此地區，目前只支援臺灣縣市的36小時內預報";
    }
}
        ...
        data.forEach(e => {
            ...
            if (loc.includes(e.locationName)) {
                ...
                for (let id = 0; id < 3; id++) {
                    const startTime = (new Date(e.weatherElement[0].time[id].startTime)).getTime();
                    const endTime = (new Date(e.weatherElement[0].time[id].endTime)).getTime();
                ...
                ret.time = e.weatherElement[0].time[id].endTime;
