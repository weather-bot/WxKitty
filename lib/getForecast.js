...
async function getForecast(msg) {
    ...
    if (taiwanArea != null) {
        ...
        // If time is in 7 days (24 hours * 7 days = 168 hours)
        }else if(deltaTime >= 60 * 60 * 36 * 1000 && deltaTime < 60 * 60 * 168 * 1000){
            const obj = await get7dayForecast(taiwanArea.name, msgTime);
            if (obj != null) {
                return parseForecastMsg(obj);
            }
        }
        return "查不到此時間，目前只支援臺灣縣市的36小時內預報。請試著縮短時間，例如明天14:00、後天早上等，或是改變時間表示法，5月23號可以寫5-23，因為套件支援英文輸入，23/5才是可辨識的英文輸入。";
    } else {
...
async function get7dayForecast(loc, msgTime) {
    try {
        ...
        var targetAreaData = data.filter(function(e){ 
            return loc.includes(e.locationName);
        })[0];
        var allTimeData = targetAreaData.weatherElement[0].time;
        var id = allTimeData.findIndex(function(e){
        ...
        var targetTimeData = allTimeData[id];

        ret = {};
        ret.area = targetAreaData.locationName;
        ret.time = msgTime.time;
        ret.weatherDescription = targetTimeData.parameterName;
