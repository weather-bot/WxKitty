...
async function isForeignAirStation(msg) {
    let result = [];
    let seg = segment(msg);
    // change to for , change timeout
    for(let i = 0; i<seg.length; i++) {
        try{
            if ( ! isPlace(seg[i]) ){
                continue;
            }
            let url = "https://wind.waqi.info/nsearch/full/" + seg[i];
            url = encodeURI(url);
            const res = await axios.get(url);
            if (res.data.results[0] != null){
                result.push({ "name": res.data.results[0].n , "x" : res.data.results[0].x }) ;//{ "name" : result[0].n , "x" : result[0].x };
            }
        }catch(err){
            console.log(err);
        }
