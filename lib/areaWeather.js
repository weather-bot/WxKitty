...
async function getAreaWeather(area) {
    let replyMsg = '';
    try {
        const googleGeoUrl = encodeURI(`https://maps.googleapis.com/maps/api/geocode/json?address=${area}&key=${config.googleMapKey}`);
        ...
        const lon = geoRes.data.results[0].geometry.location.lng;
        const lat = geoRes.data.results[0].geometry.location.lat;
        const realAreaName = geoRes.data.results[0].formatted_address;
        try {
